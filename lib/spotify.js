...
Spotify.prototype.lookup = function(type, id, format) {
  ...
  function getFullAlbumIfTrack(res) {
    var data = res.body;
  ...
  return this.client.clientCredentialsGrant()
    .then(function(data) {
      self.client.setAccessToken(data.body.access_token);
    })
    .then(function() {
      return self.client[method](id)
...
Spotify.prototype.search = function(data, format) {
  ...
  return this.client.clientCredentialsGrant()
    .then(function(tokenData) {
      self.client.setAccessToken(tokenData.body.access_token);
    })
    .then(function() {
      return self.client.searchTracks(query).then(handleResponse, handleError);
    });
};
...
var formatAlbumSlack = function(album) {
  var genres = _.isEmpty(album.genres) ? 'No genre' : album.genres.join(', ');
  return util.format('_(%s)_ (%s / %s)', album.name,
        album.release_date, genres);
};
...
Spotify.prototype.formats = {
  ...
  slack: {
    track: function(data) {
      var artists = formatArtists(data.artists);
      var track = util.format('*%s - %s*', artists, data.name);
      var album = formatAlbumSlack(data.album);
      return util.format("Track: %s %s", track, album);
    },
    album: function(data) {
      var artists = formatArtists(data.artists);
      var format = 'Album: *%s* - %s';
      return util.format(format, artists, formatAlbumSlack(data));
    },
    artist: function(data) {
      return util.format('Arist: *%s*', data.name);
    },
    trackLink: function(data) {
      var play = util.format('<%shubot/spotify/play?uri=%s|play>', heroku_url, data.uri);
      var web = util.format('<%s|web>', data.external_urls.spotify);
      var preview = util.format('<%s|preview>', data.preview_url);
      var popularity = util.format('popularity: *%s%*', data.popularity);
      return util.format('Links: %s %s %s %s %s', data.uri, play, web, preview, popularity);
    }
  }
};
